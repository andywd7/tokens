<template>
  <div>
    <div
      v-if="!hideTokenGroups.includes(token.attributes.category)"
      :style="styles"
      :class="
        `token-item${
          timeGroups.includes(token.attributes.category) ? ' hover' : ''
        }`
      "
    >
      <span v-if="textGroups.includes(token.attributes.category)">
        Aa
      </span>
    </div>
    <div>
